<script lang="ts">
	...
	type PastProject = PageData['projects'][number];
	...
	function urlFor(params: {
		...
		return `/past-projects?${searchParams.toString()}`;
	}
	...
	// Column definitions (only the requested fields)
	const columns: ColumnDef<PastProject>[] = [
		{
	...
	const tableOptions = $derived.by(() => {
		return writable<TableOptions<PastProject>>({
			data: projects as PastProject[],
			columns,
			state: {
				sorting
			},
...
<div class="space-y-4">
	...
	{#if length === 0}
		<div class="variant-soft-warning alert">
			<div>
				<strong>No past projects found</strong>
			</div>
		</div>
	{:else}
		<div class="space-y-4">
